...
#define WVMRMainThreadAssert() NSAssert([NSThread isMainThread], @"WVMRProgressHUD needs to be accessed on the main thread.");

CGFloat const WVMRProgressMaxOffset = 1000000.f;
...
@interface WVMRProgressHUDRoundedButton : UIButton
@end


@implementation WVMRProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    WVMRProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    WVMRProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (WVMRProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            WVMRProgressHUD *hud = (WVMRProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = WVMRProgressHUDAnimationFade;
    _mode = WVMRProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(WVMRProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == WVMRProgressHUDAnimationZoom) {
        type = animatingIn ? WVMRProgressHUDAnimationZoomIn : WVMRProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == WVMRProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == WVMRProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == WVMRProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == WVMRProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    WVMRProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<WVMRProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    WVMRBackgroundView *backgroundView = [[WVMRBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = WVMRProgressHUDBackgroundStyleSolidColor;
    ...
    WVMRBackgroundView *bezelView = [WVMRBackgroundView new];
    ...
    UIButton *button = [WVMRProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[WVMRRoundProgressView class]];

    WVMRProgressHUDMode mode = self.mode;
    if (mode == WVMRProgressHUDModeIndeterminate) {
    ...
    else if (mode == WVMRProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[WVMRBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == WVMRProgressHUDModeDeterminate || mode == WVMRProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[WVMRRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == WVMRProgressHUDModeAnnularDeterminate) {
            [(WVMRRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == WVMRProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == WVMRProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[WVMRProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[WVMRProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[WVMRRoundProgressView class]]) {
        WVMRRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [WVMRRoundProgressView appearanceWhenContainedIn:[WVMRProgressHUD class], nil];
#else
        appearance = [WVMRRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[WVMRProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((WVMRRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((WVMRRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[WVMRBarProgressView class]]) {
        WVMRBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [WVMRBarProgressView appearanceWhenContainedIn:[WVMRProgressHUD class], nil];
#else
        appearance = [WVMRBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[WVMRProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((WVMRBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((WVMRBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    WVMRBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(WVMRProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == WVMRProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation WVMRRoundProgressView
...
@implementation WVMRBarProgressView
...
@implementation WVMRBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = WVMRProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(WVMRProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    WVMRProgressHUDBackgroundStyle style = self.style;
    if (style == WVMRProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == WVMRProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation WVMRProgressHUDRoundedButton
